...
class LstmDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        X = torch.load('cleaned_data/seq_data/features/' + ID + '.pt')
        y = torch.load('cleaned_data/seq_data/labels/' + ID + '.pt')
        full_seq_info = np.load('cleaned_data/seq_data/full_seq_info/' + ID + '.npy')
        seq_length = int(full_seq_info[1])

        return X, y, seq_length, ID
...
class SortedDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        # Load data and get label
        X = torch.load('cleaned_data/seq_sorted_data/features/' + ID + '.pt')
        y = torch.load('cleaned_data/seq_sorted_data/labels/' + ID + '.pt')
        full_seq_info = np.load('cleaned_data/seq_sorted_data/full_seq_info/' + ID + '.npy')
        seq_length = int(full_seq_info[1])

        return X, y, seq_length, ID
  
'''
data_dir = [seq_data, seq_sorted_data, seq_filtered_sorted_data]

seq_filtered_sorted_data : X = [batch_size, max_seq_len, 24*9]
'''
class SeqDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        full_seq_info = np.load(f'/Users/scottmaran/Code/Python/bdb_2024/cleaned_data/{self.data_dir}/full_seq_info/' + ID + '.npy')
        seq_length = int(full_seq_info[1])
        player_ids = full_seq_info[-23:]

        return X, y, seq_length, player_ids, ID
...
class SingleFrameDataset(torch.utils.data.Dataset):
  # takes as input json file
  def __init__(self, features : torch.Tensor, labels : torch.Tensor, player_ids : torch.Tensor):
  ...
  def __getitem__(self, index):
        'Generates one sample of data'
        return self.features[index], self.labels[index], [], self.player_ids[index], []
...
# Flip single frame over y-axis

# ['attacking_team','football','ball_carrier','x_adj','y_adj','dir_adj','dir_o', 's', 'a']
# """
# class FlipFrame(object):
#       def __call__(self, sample):
#         # batch = (207)
#         (x, local_labels, local_lengths, local_ids) = sample
        
#         flipped_y = x.reshape(23,-9)

        
#         return new_x, local_labels, local_lengths, local_ids
